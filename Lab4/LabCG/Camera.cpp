...
void Camera::drawface(vector<vector<float>> polyverts, float param, vector<vector<float>> normals, Model* model, Shader shaderType)
{
			...
			for (float x = xa; x <= xb; x++) {
				...
				if (zbuffer[y * width + int(x)] > zp&& x > 0 && x < width && zp > 0 && zp < 1) {
					...
					if (shaderType == Phong || shaderType == Flat) {
						...
						float u = 0.5 + atan2(N[2], -N[0]) / PI / 2;
						float v = 0.5 - asin(N[1]) / PI;
						vector<float> R = vNormalizeVector(vvSubVector(vvMultVector(N, 2 * vvDotVector(N, light_dir)), light_dir));
						float specular = pow(max(vvDotVector(vNormalizeVector(cameraVector), R), 0.0f), 128);
						//vector<float> H = vNormalizeVector(vvAddVector(light_dir, vNormalizeVector(cameraVector)));
						//float specular = pow(max(vvDotVector(N, H), 0.0f), 256);
						float diffuse = max(vvDotVector(N, light_dir), 0.0f);
						float texture_specular = model->texture2D(u, v, model->specular);
						float texture_cloud = model->texture2D(u, v, model->cloud);
						float texture_lights = model->texture2D(u, v, model->lights);
						pixelColor = {
							model->texture2D(u, v, model->texureR) * (diffuse + texture_specular * specular * 6) + texture_cloud * (diffuse + 0.06f) + max(texture_lights * (0.6f - diffuse) * 2.0f, 0.0f),
							model->texture2D(u, v, model->texureG) * (diffuse + texture_specular * specular * 4) + texture_cloud * (diffuse + 0.06f) + max(texture_lights * (0.6f - diffuse) * 1.7f , 0.0f),
							model->texture2D(u, v, model->texureB) * (diffuse + texture_specular * specular * 2) + texture_cloud * (diffuse + 0.06f) + max(texture_lights * (0.6f - diffuse) * 1.0f , 0.0f) };
					}
	...
	for (int m = 0; m < models.size(); m++) {
		for (int i = 0; i < models[m]->faces.size(); i++) {
			...
			for (int j = 0; j < pointsPerFace.size(); j++) {
				...
				if (shaderType == Gouraud) {
					vector<float> N = models[m]->getNormal(i, j);
					N = vNormalizeVector(N);
					float u = 0.5 + atan2(N[2], -N[0]) / PI / 2;
					float v = 0.5 - asin(N[1]) / PI;
					light_dir = vNormalizeVector(light_dir);
					float vertex_intensity = vvDotVector(light_dir, N);
					vector<float> H = vNormalizeVector(vvAddVector(light_dir, vNormalizeVector(cameraVector)));
					float vertex_specular = pow(max(vvDotVector(H, N), 0.0f), 128);
					float texture_specular = models[m]->texture2D(u, v, models[m]->specular);
					float texture_cloud = models[m]->texture2D(u, v, models[m]->cloud);
					float texture_lights = models[m]->texture2D(u, v, models[m]->lights);
					vector<float> vertex_color = {
						models[m]->texture2D(u, v, models[m]->texureR) * (vertex_intensity + vertex_specular * 2) + texture_cloud * (vertex_intensity + 0.06f) + max(texture_lights * (0.6f - vertex_intensity) * 2.0f, 0.0f),
						models[m]->texture2D(u, v, models[m]->texureG) * (vertex_intensity + vertex_specular * 2) + texture_cloud * (vertex_intensity + 0.06f) + max(texture_lights * (0.6f - vertex_intensity) * 1.7f, 0.0f),
						models[m]->texture2D(u, v, models[m]->texureB) * (vertex_intensity + vertex_specular * 2) + texture_cloud * (vertex_intensity + 0.06f) + max(texture_lights * (0.6f - vertex_intensity) * 1.0f, 0.0f), };
			...
			if (shaderType == Phong)
				drawface(screenPointsPerFace, dotVal, normalsPerFace, models[m], Phong);		// Phong
			if (shaderType == Gouraud)
				drawface(screenPointsPerFace, dotVal, colorsPerFace, models[m], Gouraud);	// Gourand
			if (shaderType == Flat) {
				drawface(screenPointsPerFace, 1, normalsPerFace, models[m], Flat);	// Flat
			}
...
void Camera::loadImage() {
	ifstream in("Textures/starBG.txt");
	if (in.fail())
		return;
	string line;
	bgImg = new float[width * width];
	for (int y = 0; y < width; y++) {
		for (int x = 0; x < width; x++) {
			getline(in, line);
			bgImg[y * width + x] = stof(line);
		}
	}
	in.close();
}
